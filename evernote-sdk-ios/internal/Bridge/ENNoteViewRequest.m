...
+ (ENNoteViewRequest *)noteViewRequestWithNoteID:(NSString *)noteID
                                     searchTerms:(NSString *)searchTerms {
  ENNoteViewRequest *result = [[ENNoteViewRequest alloc] init];
  result.noteID = noteID;
...
+ (ENNoteViewRequest *)noteViewRequestWithURL:(NSURL *)url
  ...
  // Note Link specifications:
  // https://docs.google.com/a/evernote.com/document/d/1TmG-_dgM-rJKckedAubrg9RpS4lNTFr_IhlATSPDKRc/edit?hl=en
  // URL example:
  // evernote:///view/user_id/s1/b87917eb-ee0e-464f-9ec8-fb29fb0f93da/x-coredata%3A%2F%2FAABB6B76-F1F2-4645-83F6-79C643C61D50%2FENNote%2Fp68/

  // NSString *requestPath = [url path];   // this returns unescaped string with
  // stringByReplacingPercentEscapesUsingEncoding: then mess up
  // clientSpecificId!
  ...
  if (components.count != 9) {
    if (outError != nil) {
      NSDictionary *userInfo = [NSDictionary
          dictionaryWithObjectsAndKeys:NSLocalizedString(@"Error", @"Error"),
                                       NSLocalizedFailureReasonErrorKey,
                                       NSLocalizedString(
                                           @"NoteLinkIsNotValidFormat",
                                           @"NoteLinkIsNotValidFormat"),
...
- (id)initWithCoder:(NSCoder *)coder {
  ...
  if (self != nil) {
    ...
    if (encodedVersion != ENNoteViewRequestVersion1 &&
        encodedVersion != ENNoteViewRequestVersion2 &&
        encodedVersion != ENNoteViewRequestVersion3) {
    ...
    if (encodedVersion >= ENNoteViewRequestVersion2) {
      self.searchTerms =
          [coder decodeObjectForKey:kEN_NoteViewRequest_SearchTerms];
    }
    if (encodedVersion >= ENNoteViewRequestVersion3) {
      self.linkedNotebookID =
          [coder decodeObjectForKey:kEN_NoteViewRequest_LinkedNotebookID];
    }
  }
  return self;
}
...
- (void)encodeWithCoder:(NSCoder *)coder {
  [coder encodeInt:ENNoteViewRequestCurrentVersion
            forKey:kEN_NoteViewRequest_VersionKey];
  ...
  [coder encodeObject:_linkedNotebookID
               forKey:kEN_NoteViewRequest_LinkedNotebookID];
  [coder encodeObject:_shardID forKey:kEN_NoteViewRequest_ShardID];
  [coder encodeInteger:_userID forKey:kEN_NoteViewRequest_UserID];
}
