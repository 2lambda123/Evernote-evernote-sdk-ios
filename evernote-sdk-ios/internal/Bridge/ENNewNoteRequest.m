...
static NSString *kEN_NewNoteRequest_ResourceAttachments =
    @"mResourceAttachments";
...
- (id)initWithCoder:(NSCoder *)coder {
  ...
  if (self != nil) {
    ...
    self.contentMimeType =
        [coder decodeObjectForKey:kEN_NewNoteRequest_ContentMimeType];
    self.sourceURL = [coder decodeObjectForKey:kEN_NewNoteRequest_SourceURL];
    self.sourceApplication =
        [coder decodeObjectForKey:kEN_NewNoteRequest_SourceApplication];
    self.tagNames = [coder decodeObjectForKey:kEN_NewNoteRequest_TagNames];
    self.resourceAttachments =
        [coder decodeObjectForKey:kEN_NewNoteRequest_ResourceAttachments];
    ...
    self.reminderDoneTime =
        [coder decodeObjectForKey:kEN_NewNoteRequest_ReminderDoneTime];
    self.reminderOrder =
        [coder decodeObjectForKey:kEN_NewNoteRequest_ReminderOrder];
    self.reminderTime =
        [coder decodeObjectForKey:kEN_NewNoteRequest_ReminderTime];

    self.notebookGUID =
        [coder decodeObjectForKey:kEN_NewNoteRequest_NotebookGUID];
  }
  return self;
}

- (void)encodeWithCoder:(NSCoder *)coder {
  [coder encodeInt:kEN_NewNoteRequest_Version
            forKey:kEN_NewNoteRequest_VersionKey];
  ...
  [coder encodeObject:self.contentMimeType
               forKey:kEN_NewNoteRequest_ContentMimeType];
  [coder encodeObject:self.sourceURL forKey:kEN_NewNoteRequest_SourceURL];
  [coder encodeObject:self.sourceApplication
               forKey:kEN_NewNoteRequest_SourceApplication];
  [coder encodeObject:self.tagNames forKey:kEN_NewNoteRequest_TagNames];
  [coder encodeObject:self.resourceAttachments
               forKey:kEN_NewNoteRequest_ResourceAttachments];
  [coder encodeObject:self.reminderDoneTime
               forKey:kEN_NewNoteRequest_ReminderDoneTime];
  [coder encodeObject:self.reminderOrder
               forKey:kEN_NewNoteRequest_ReminderOrder];
...
- (void)addResourceAttachment:(ENResourceAttachment *)resourceAttachment {
  NSMutableArray *resourceAttachments =
      [NSMutableArray arrayWithArray:self.resourceAttachments];
    ...
    if (resourceData != nil) {
      ...
      if (filepath != nil) {
        ...
        if (fileAttributes == nil) {
          NSLog(@"%s attributesOfItemAtPath:%@ error:%@", __PRETTY_FUNCTION__,
                filepath, attrError);
        } else {
