...
- (void)viewDidLoad {
  ...
  [self.activityIndicator
      setFrame:CGRectMake(viewRect.size.width / 2, viewRect.size.height / 2, 20,
                          20)];
...
- (void)loadMoreNotes {
  ...
  EDAMNotesMetadataResultSpec *resultSpec =
      [[EDAMNotesMetadataResultSpec alloc] initWithIncludeTitle:NO
                                           includeContentLength:NO
                                                 includeCreated:NO
                                                 includeUpdated:NO
                                                 includeDeleted:NO
                                       includeUpdateSequenceNum:NO
                                            includeNotebookGuid:NO
                                                includeTagGuids:NO
                                              includeAttributes:NO
                                     includeLargestResourceMime:NO
                                     includeLargestResourceSize:NO];
  [[EvernoteNoteStore noteStore] findNotesMetadataWithFilter:filter
      offset:(int)self.currentNote
      maxNotes:10
...
- (void)loadCurrentNote {
  ...
  if ([self.noteList count] > self.currentNote % 10) {
    ...
    [[EvernoteNoteStore noteStore] getNoteWithGuid:foundNote.guid
        withContent:YES
        ...
        success:^(EDAMNote *note) {
          ...
          [utltility convertENMLToHTML:note.content
                         withResources:note.resources
                       completionBlock:^(NSString *html, NSError *error) {
                         if (error == nil) {
