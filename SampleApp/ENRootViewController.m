      ...
      authenticateWithViewController:self
                   completionHandler:^(NSError *error) {
                     if (error || !session.isAuthenticated) {
                       ...
                       NSLog(
                           @"authenticated! noteStoreUrl:%@ webApiUrlPrefix:%@",
                           session.noteStoreUrl, session.webApiUrlPrefix);
                       [self updateButtonsForAuthentication];
                     }
                   }];
}
...
- (IBAction)createBusinessNotebook:(id)sender {
  EvernoteNoteStore *noteStore = [EvernoteNoteStore noteStore];
  EDAMNotebook *notebook = [[EDAMNotebook alloc] initWithGuid:nil
                                                         name:@"test"
                                            updateSequenceNum:0
                                              defaultNotebook:NO
                                               serviceCreated:0
                                            ...
                                            sharedNotebookIds:nil
                                              sharedNotebooks:nil
                                             businessNotebook:nil
                                                      contact:nil
                                                 restrictions:nil];
  [noteStore createBusinessNotebook:notebook
      success:^(EDAMLinkedNotebook *businessNotebook) {
  ...
  NSString *noteContent = [NSString
      stringWithFormat:
          @"<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
           ...
           "%@"
           "</en-note>",
          [ENMLUtility mediaTagWithDataHash:dataHash mime:@"image/png"]];
