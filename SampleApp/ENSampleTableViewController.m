...
- (void)viewDidLoad {
  ...
  [self.activityIndicatorView
      setCenter:CGPointMake(self.view.frame.size.width / 2,
                            (self.view.frame.size.height - 20) / 2)];
...
- (IBAction)listNotes {
  ...
  [noteStore
      listNotebooksWithSuccess:^(NSArray *notebooks) {
        self.consoleText =
            [NSString stringWithFormat:@"notebooks: %@", notebooks];
      ...
      listLinkedNotebooksWithSuccess:^(NSArray *linkedNotebooks) {
        if (linkedNotebooks.count > 0) {
          EDAMNoteFilter *noteFilter =
              [[EDAMNoteFilter alloc] initWithOrder:0
                                          ascending:NO
                                              words:nil
                                       notebookGuid:nil
                                           tagGuids:nil
                                           ...
                                           inactive:NO
                                         emphasized:nil];
          [defaultNoteStore listNotesForLinkedNotebook:linkedNotebooks[0]
              withFilter:noteFilter
              success:^(EDAMNoteList *list) {
                self.consoleText =
                    [NSString stringWithFormat:@"Shared notes : %@", list];
                [self.activityIndicatorView stopAnimating];
                [self logToConsole];
              }
              failure:^(NSError *error) {
...
- (IBAction)createPhotoNote {
  NSString *filePath =
      [[NSBundle mainBundle] pathForResource:@"evernote_logo_4c-sm"
                                      ofType:@"png"];
  ...
  [[EvernoteNoteStore noteStore]
      setUploadProgressBlock:^(NSUInteger bytesWritten,
        ...
        NSLog(@"Total bytes written : %lld , Total bytes expected to be "
              @"written : %lld",
              totalBytesWritten, totalBytesExpectedToWrite);
      }];
  NSLog(@"Contents : %@", myWriter.contents);
  [self.activityIndicatorView startAnimating];
  [[EvernoteNoteStore noteStore] createNote:newNote
...
- (IBAction)createReminderNote {
  ...
  // Set the reminder
  EDAMNoteAttributes *noteAttributes =
      [[EDAMNoteAttributes alloc] initWithSubjectDate:0
                                             latitude:0
                                            longitude:0
                                             altitude:0
                                               author:nil
                                    ...
                                    sourceApplication:nil
                                            shareDate:0
                                        reminderOrder:[now enedamTimestamp]
                                     reminderDoneTime:0
                                         reminderTime:[then enedamTimestamp]
                                            placeName:nil
                                         contentClass:nil
                                      applicationData:nil
                                         lastEditedBy:nil
  ...
  // Create note object
  EDAMNote *ourNote = [[EDAMNote alloc] initWithGuid:nil
  ...
  [[EvernoteNoteStore noteStore] createNote:ourNote
      success:^(EDAMNote *note) {
...
- (IBAction)saveNewNote {
  if ([[EvernoteSession sharedSession] isEvernoteInstalled]) {
    ...
    NSData *dataHash = [myFileData enmd5];
    EDAMData *edamData =
        [[EDAMData alloc] initWithBodyHash:dataHash
                                      size:(int)myFileData.length
                                      body:myFileData];
    EDAMResource *resource = [[EDAMResource alloc] initWithGuid:nil
                                                       noteGuid:nil
    ...
    NSMutableArray *resources =
        [NSMutableArray arrayWithObjects:resource, resource, nil];
    NSMutableArray *tagNames =
        [NSMutableArray arrayWithObjects:@"evernote", @"sdk", nil];
    // Include NSDate+EDAMAdditions.h
    NSDate *now = [NSDate date];
    // After 60 minutes
    NSDate *then = [now dateByAddingTimeInterval:3600];
    ...
    EDAMNote *note = [[EDAMNote alloc]
             initWithGuid:nil
                  ...
                  content:@"<strong>Here is my new HTML note</strong>"
              contentHash:nil
            contentLength:0
                  created:0
                  updated:0
    ...
    [[EvernoteNoteStore noteStore] saveNewNoteToEvernoteApp:note
                                                   withType:@"text/html"];
  } else {
    [self installEvernote];
  }
...
- (IBAction)viewNote {
  if ([[EvernoteSession sharedSession] isEvernoteInstalled]) {
    NSLog(@"Viewing note..");
    EDAMNoteFilter *filter = [[EDAMNoteFilter alloc] initWithOrder:0
                                                         ascending:NO
                                                             words:nil
                                                      notebookGuid:nil
                                                          tagGuids:nil
...
- (IBAction)invlokeNotebookChooser {
  NotebookChooserViewController *nbc =
      [[NotebookChooserViewController alloc] init];
  [nbc setDelegate:self];
  [nbc setSelectedNotebookWithGUID:self.selectedNotebookGUID];
  UINavigationController *notbookChooserNav =
      [[UINavigationController alloc] initWithRootViewController:nbc];
  ...
  [[self navigationController]
      presentViewController:notbookChooserNav
                   animated:YES
                 completion:^{
...
- (void)notebookChooserController:(NotebookChooserViewController *)controller
                didSelectNotebook:(EDAMNotebook *)notebook {
  self.selectedNotebookGUID = notebook.guid;
  UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Notebook chosen"
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([segue.identifier isEqualToString:@"ShowConsole"]) {
    ...
    NotebookTableViewController *notebookTableView =
